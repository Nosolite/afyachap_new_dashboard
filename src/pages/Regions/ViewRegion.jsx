...
import { districtsHeadCells } from '../../seed/table-headers';
import PencilIcon from '@heroicons/react/24/outline/PencilIcon';
import TrashIcon from '@heroicons/react/24/outline/TrashIcon';
import { DeleteDialog } from '../../components/delete-dialog';
import { CREATE, UPDATE, filterItems } from '../../utils/constant';
import { FormDialog } from '../../components/form-dialog';
import { districtsFields } from '../../seed/form-fields';
import { postRequest } from '../../services/api-service';
import { addDistrictUrl, deleteDistrictUrl, getAllDistrictsByPaginationUrl, updateDistrictUrl } from '../../seed/url';
...
function ViewRegion({ open, handleClose, selected }) {
    ...
    const [openViewWardsDialog, setOpenViewWardsDialog] = React.useState(false);
    ...
    const values = [
        {
            ...
            district_name: action === UPDATE ? productsRatingsSelection.selected[0].district_name : ""
        }
    ]
    ...
    const handleClickOpenViewWardsDialog = () => {
        setOpenViewWardsDialog(true)
    }

    const handleCloseViewWardssDialog = () => {
        setOpenViewWardsDialog(false)
    }
    ...
    const contentPopoverItems = [
        {
            ...
            onClick: () => {
                if (productsRatingsSelection?.selected[0]?.id) {
                    handleClickOpenViewWardsDialog()
                }
            },
        },
        {
                ...
                <>
                    ...
                    {openCreateDialog &&
                        <FormDialog
                            ...
                            values={values}
                            url={action === CREATE ? addDistrictUrl : updateDistrictUrl}
                        />
                    }
                    {openViewWardsDialog &&
                        <ViewDistrict
                            open={openViewWardsDialog}
                            handleClose={handleCloseViewWardssDialog}
                            selected={productsRatingsSelection.selected[0]}
                        />
                    }
                    {openDeleteDialog &&
                        <DeleteDialog
                                ...
                                >
                                    <Stack spacing={1}>
                                        <Typography variant="h4">
                                            Districts Of {selected.region_name}
                                        </Typography>
                                    </Stack>
                                    <div>
                                        <Button
