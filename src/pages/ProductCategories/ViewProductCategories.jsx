...
import { productSubCategoriesHeadCells } from '../../seed/table-headers';
import PencilIcon from '@heroicons/react/24/outline/PencilIcon';
import TrashIcon from '@heroicons/react/24/outline/TrashIcon';
import { DeleteDialog } from '../../components/delete-dialog';
import { CREATE, UPDATE, filterItems } from '../../utils/constant';
import { FormDialog } from '../../components/form-dialog';
import { productSubCategoriesFields } from '../../seed/form-fields';
import { postRequest } from '../../services/api-service';
import { createProductsSubCategoryUrl, deleteProductsSubCategoryUrl, getAllProductsSubCategoriesByPaginationUrl, updateProductsSubCategoryUrl } from '../../seed/url';
...
function ViewProductCategories({ open, handleClose, selected }) {
    ...
    const productSubCategorySelection = useSelection(specializationsIds);
    ...
    const values = [
        {
            id: action === UPDATE ? productSubCategorySelection.selected[0].id : 0,
            product_category_id: selected.id,
            product_sub_category_name: action === UPDATE ? productSubCategorySelection.selected[0].product_sub_category_name : "",
            image: action === UPDATE ? productSubCategorySelection.selected[0].icon_url : null
        }
    ]
    ...
    const fetcher = React.useCallback(
        (page) => {
            postRequest(
                getAllProductsSubCategoriesByPaginationUrl,
                {
                    "query": searchTerm,
                    "product_category_id": selected.id,
                    "sort": orderBy + " " + order,
    ...
    const handleDelete = async () => {
        postRequest(
            deleteProductsSubCategoryUrl,
            {
                id: productSubCategorySelection.selected[0].id,
            },
            (data) => {
                ...
                <>
                    ...
                    {openCreateDialog &&
                        <FormDialog
                            ...
                            action={action}
                            fields={productSubCategoriesFields}
                            values={values}
                            url={action === CREATE ? createProductsSubCategoryUrl : updateProductsSubCategoryUrl}
                        />
                    }
                    {openDeleteDialog &&
                        <DeleteDialog
                        ...
                        <Container maxWidth="xl">
                            <Stack spacing={2}>
                                ...
                                >
                                    <Stack spacing={1}>
                                        <Typography variant="h4">
                                            Sub Categories Of {selected.product_category_name}
                                        </Typography>
                                    </Stack>
                                    <div>
                                        <Button
                                ...
                                <CustomTable
                                    ...
                                    onSelectOne={productSubCategorySelection.handleSelectOne}
                                    page={specializations.page >= 1 ? specializations.page - 1 : specializations.page}
                                    rowsPerPage={rowsPerPage}
                                    selected={productSubCategorySelection.selected}
                                    headCells={productSubCategoriesHeadCells}
